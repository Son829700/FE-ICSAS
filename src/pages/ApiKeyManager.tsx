...
// --- Icon Components (Tách ra để code chính gọn hơn) ---
const AddIcon = () => (
...
const CheckIcon = () => (
  <svg className="fill-current text-green-500" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function ApiKeyManager() {
  ...
  const [apiKeys, setApiKeys] = useState([
    {
      ...
      keyPrefix: "sk_live_",
      maskedKey: "**********4248",
      fullKey: "sk_live_12345678901234248", // Key thực tế (để copy)
      status: "Disabled",
      created: "25 Jan, 2025",
    ...
    },
    {
      ...
      keyPrefix: "dev_live_",
      maskedKey: "**********4923",
      fullKey: "dev_live_98765432109874923",
      status: "Active",
      created: "29 Dec, 2024",
    ...
    },
    {
      ...
      keyPrefix: "leg_live_",
      maskedKey: "**********0932",
      fullKey: "leg_live_56473829104830932",
      status: "Active",
      created: "12 Mar, 2024",
  ...
  // State để hiển thị thông báo "Copied" cho ID cụ thể
  ...
  const handleAddApiKey = () => {
    ...
    const randomNum = Math.floor(1000 + Math.random() * 9000); // Random 4 số cuối
    const newKey = {
      id: newId,
      name: `New API Key ${newId}`,
      keyPrefix: "sk_test_",
      maskedKey: `**********${randomNum}`,
      fullKey: `sk_test_${Date.now()}${randomNum}`,
      status: "Active",
      created: "Just now",
      lastUsed: "Never",
    };
  ...
  const handleCopy = (text: string, id: number | React.SetStateAction<null>) => {
  ...
  const handleRegenerate = (id: number) => {
    // Logic thực tế sẽ gọi API. Ở đây mô phỏng tạo số đuôi mới.
    ...
    setApiKeys(apiKeys.map(item => {
      if (item.id === id) {
        return {
          ...item,
          maskedKey: `**********${newSuffix}`,
          fullKey: `${item.keyPrefix}regenerated${newSuffix}`,
          created: "Just now" // Update ngày tạo nếu cần
        };
      }
      return item;
    }));
  ...
  const handleEditName = (id) => {
    const itemToEdit = apiKeys.find(k => k.id === id);
    const newName = window.prompt("Enter new name for API Key:", itemToEdit.name);
    if (newName && newName.trim() !== "") {
        setApiKeys(apiKeys.map(item => item.id === id ? {...item, name: newName} : item));
    }
  };

  return (
    <div className="rounded-2xl border border-gray-200 bg-white px-6 pl-5 dark:border-gray-800 dark:bg-white/5">
      {/* Header */}
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center justify-between border-b border-gray-100 py-4 dark:border-gray-800">
        ...
        <div>
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg transition px-5 py-3.5 text-sm bg-blue-600 text-white shadow-theme-xs hover:bg-blue-700 disabled:bg-blue-300"
          >
            <AddIcon />
            Add API Key
          </button>
      ...
      <div className="custom-scrollbar overflow-x-auto px-1 pb-4">
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {apiKeys.map((item) => (
              <tr key={item.id}>
                {/* Name & Key Input Column */}
                <td className="py-3 pr-5 whitespace-nowrap">
                  <div>
                    <label htmlFor={`api-${item.id}`} className="mb-2 inline-block text-sm text-gray-700 dark:text-gray-400">
                      {item.name}
                    </label>
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <input
                          id={`api-${item.id}`}
                          className="dark:bg-dark-900 shadow-theme-xs focus:border-blue-300 focus:ring-blue-500/10 dark:focus:border-blue-800 h-11 w-full min-w-[360px] rounded-lg border border-gray-300 bg-transparent py-3 pr-[90px] pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                          readOnly
                          type="text"
                          value={`${item.keyPrefix}${item.maskedKey}`}
                        />
                        <button
                          onClick={() => handleCopy(item.fullKey, item.id)}
                          className="absolute top-1/2 right-0 inline-flex h-11 -translate-y-1/2 cursor-pointer items-center gap-1 rounded-r-lg border border-gray-300 py-3 pr-3 pl-3.5 text-sm font-medium text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                        >
                          {copiedId === item.id ? <CheckIcon /> : <CopyIcon />}
                          <div id="copy-text">{copiedId === item.id ? "Copied" : "Copy"}</div>
                        </button>
                      </div>
                      ...
                      <div className="group relative inline-block">
                        <button 
                            ...
                            className="inline-flex h-11 w-11 items-center justify-center rounded-lg border border-gray-300 text-gray-700 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
                        >
                          <RegenerateIcon />
                        </button>
                        <div className="invisible absolute bottom-full left-1/2 z-50 mb-2.5 -translate-x-1/2 opacity-0 transition-opacity duration-300 group-hover:visible group-hover:opacity-100">
                          <div className="relative">
                            <div className="rounded-lg bg-white px-3 py-2 text-xs font-medium whitespace-nowrap text-gray-700 shadow-lg dark:bg-[#1E2634] dark:text-white border dark:border-gray-700">
                              Regenerate
                            </div>
                            <div className="absolute -bottom-1 left-1/2 h-3 w-4 -translate-x-1/2 rotate-45 bg-white dark:bg-[#1E2634] border-b border-r dark:border-gray-700"></div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <div
                        className={`block transition duration-150 ease-linear h-6 w-11 rounded-full ${
                          item.status === "Active" ? "bg-blue-600" : "bg-gray-200 dark:bg-white/10"
                        }`}
                      ></div>
                      <div
                        className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full shadow-sm duration-150 ease-linear transform bg-white ${
                          item.status === "Active" ? "translate-x-full" : "translate-x-0"
                        }`}
                      ></div>
                    </div>
                  </label>
                ...
                <td className="px-5 py-3 whitespace-nowrap">
                  <div className="flex w-full items-center gap-3">
                    <button 
                        ...
                        className="hover:text-red-500 dark:hover:text-red-500 text-gray-500 dark:text-gray-400 transition"
                        title="Delete"
                    >
                      <TrashIcon />
                    </button>
