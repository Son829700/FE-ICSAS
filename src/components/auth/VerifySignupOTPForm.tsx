...
interface Props {
  formData: { user_name: string; email: string; password: string };
  onBack: () => void;
}

export default function VerifySignupOTPForm({ formData, onBack }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Account created successfully! Please wait for admin activation.", { duration: 5000 });
      navigate("/signin");
    } catch (err: any) {
